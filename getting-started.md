...
<section class="chapter-quick-start chapter-quick-start--single" aria-labelledby="getting-started-quick-start">
  <div>
    ...
    <p class="chapter-quick-start__done"><strong>完成時：</strong>BGM 已載入並會自動循環；雙擊歌曲後，伴奏播放器會顯示歌名與進度，BGM 也會自動暫停。儲存專案後，視窗標題的 `*` 會消失。</p>
  </div>
</section>

{% include first-stream-check.html %}
...
> 第一次使用不必先調整介面語言、專案／媒體資料夾位置、封面或 OBS WebSocket。先完成 BGM、伴奏及一次測試播放，就能在本機使用；準備直播時，再依[OBS 聲音與畫面檢查](#first-stream-check)完成短錄影。歌單與歌詞可依需要設定。
