...
try{
  let tab;
  for(let i=0;i<60;i++){
    if(chrome.exitCode!==null)throw Error('Chrome exited');
  ...
  assert.ok(tab?.webSocketDebuggerUrl,'Chrome DevTools required');
  ...
  for(const [lang,data] of Object.entries(copy))for(const width of [390,1440])for(const theme of ['light','dark']){
    ...
    if(lang==='zh-TW'&&theme==='light'){
      ...
      await evaluate("document.querySelector('details.audio-route-details--installation').scrollIntoView({block:'center'})");
