...
description: Singing Stream Savior 2.1.3.2 한국어 설명서
lang: ko
...
Singing Stream Savior는 노래 방송을 위한 Windows 앱입니다. 곡 라이브러리, BGM, 반주, 대기 목록, 가사, 보컬 처리, 오디오 라우팅과 OBS 출력을 하나의 흐름으로 관리합니다. 이 설명서는 **2.1.3.2** 버전을 기준으로 합니다.
...
> **음원과 재생 용도:** 곡 라이브러리에 추가하는 것은 재생 항목을 만드는 기능이며 음악 이용 허락을 제공하지 않습니다. 소재의 출처와 이용 조건을 확인하세요. 개인 감상, 음원 분리, 라이브 방송, 녹화물 보관과 출력 파일 공유에는 서로 다른 범위의 권리가 필요할 수 있습니다. YouTube 음원에는 해당 플랫폼 규정도 적용됩니다.
...
표지는 필수가 아니며 Card와 CD 테마에서 특별히 활용됩니다.

> **표지 이미지 이용:** 검색 기능은 이미지를 찾는 데 도움을 줄 뿐입니다. 공개 방송이나 영상에서 사용하기 전에 필요한 이용 권한을 확인하세요. 직접 제작했거나 적절한 허락을 받은 이미지를 사용할 수도 있습니다.
...
대기 목록이 비어 있을 때 대기 목록 모드로 재생을 눌러도 안내만 표시되고 BGM은 중단되지 않습니다. 곡 라이브러리에서 직접 두 번 클릭하면 자동으로 **한 번 재생**으로 돌아갑니다. 재생목록 외관에서 OBS 대기 목록 표시를 켠 뒤 **다음 한 곡만** 또는 2, 3, 5, 10곡을 선택할 수 있습니다. 정상적으로 앱을 종료하면 부른 곡 기록은 다음 방송으로 넘어가지 않으며, 예기치 않게 중단된 경우에는 복구 스냅샷으로 해당 라이브 세션의 진행 상황을 복원할 수 있습니다.

<a id="bgm-chat-topic"></a>
### BGM 재생 중 대화 주제 표시

BGM 목록 버튼 옆의 대화 주제 설정을 열고 BGM 재생 중 표시 옵션을 켜세요. 「잡담 중」 같은 제목을 입력하고 필요하면 주제도 추가합니다. 제목이 비어 있으면 기본 문구가 사용됩니다. 변경은 즉시 적용되며 프로젝트와 함께 저장됩니다.

OBS 재생목록 테마의 현재 곡 영역이 대화 문구로 바뀌는 것은 **BGM 재생 중일 때만**입니다. BGM을 일시정지하거나 정지하면 숨겨집니다. 반주나 무반주 공연이 진행 중이면 곡명이 우선 표시됩니다. 곡명 데이터, 대기 목록, 부른 곡 기록은 바뀌지 않습니다. 필요 없으면 표시 옵션을 끄세요.

<figure class="manual-figure manual-figure--compact">
  <a href="{{ '/assets/images/bgm-chat-topic.png' | relative_url }}"><img src="{{ '/assets/images/bgm-chat-topic.png' | relative_url }}" alt="BGM 대화 주제 설정. 사진에서는 표시 옵션이 꺼져 있습니다(번체 중국어 UI)." loading="lazy" decoding="async"></a>
...
**2.1.3.2**부터 Basic Lyrics, Classic Karaoke, Vertical Verse의 배치를 편집할 수 있습니다. 「라이브 조작 → 가사」에서 스타일을 고른 뒤 미리보기 위의 레이아웃 편집 버튼을 누르세요. 블록 테두리 안쪽을 드래그하면 이동하고, 모서리 핸들을 드래그하면 비율을 유지하며 크기가 바뀝니다. 캔버스는 **1920 × 1080(16:9)**으로 유지됩니다.
...
「재생목록 외관」에서 테마를 선택하고 미리보기 위의 레이아웃 편집 버튼을 누르세요. 테두리 안쪽을 드래그해 이동하거나 모서리 핸들로 비율을 유지하며 확대·축소합니다. 캔버스는 **1920 × 1080(16:9)**으로 고정되며, 같은 출력을 사용하는 OBS 소스에는 **드래그 중에도 반영**됩니다.

중앙 정렬 안내선을 사용할 수 있습니다. 버튼을 다시 누르거나 페이지를 떠나면 편집만 종료되고 위치는 유지됩니다. 옆의 초기화 아이콘으로 배치를 되돌리고, 프로젝트를 저장해 설정을 보관하세요. 고정 디자인 테마도 전체 이동과 크기 조절이 가능하지만 내부 글꼴, 색상, 문자 배치는 테마가 지원하는 옵션에 따릅니다.

<figure class="manual-figure">
  <a href="{{ '/assets/images/playlist-layout-editor-2132.png' | relative_url }}"><img src="{{ '/assets/images/playlist-layout-editor-2132.png' | relative_url }}" alt="2.1.3.2 재생목록 배치 편집 화면(번체 중국어 UI)." loading="lazy" decoding="async"></a>
  <figcaption>2.1.3.2 재생목록 배치 편집 화면(번체 중국어 UI).</figcaption>
</figure>
...
OBS Studio 28 이상에서 **도구 > WebSocket 서버 설정**을 열고 서버를 켠 뒤 포트(보통 `4455`)와 비밀번호를 확인합니다. 연결 정보 창을 닫은 뒤 **확인**을 눌러 저장하고 설정을 닫으세요. Singing Stream Savior의 **설정 > 방송 타임스탬프**에서 WebSocket을 켜고 `127.0.0.1`, 같은 포트와 비밀번호를 입력한 다음 **연결**을 누르세요. OBS 비밀번호는 로컬 연결용 인증 정보이므로 비밀번호가 보이는 설정 화면을 공개하지 마세요.
...
> **시작하기 전에 소재의 이용 범위를 확인하세요.**
>
> 관련 권리를 보유한 소재, 음원 분리까지 허락된 소재 또는 법에 따라 이용할 수 있는 소재를 사용하세요. 음원을 구입했거나 온라인에서 재생할 수 있거나 다른 사람이 공유했다는 사실만으로 분리, 방송, 업로드 또는 배포할 수 있는 것은 아닙니다.
>
> 이 기능은 음악 이용 허락을 제공하지 않습니다. 분리 결과에도 보호받는 곡이나 녹음이 포함될 수 있습니다. 공개 사용 전에는 보유한 허락이 예정한 용도를 포함하는지 확인하세요. YouTube 음원을 사용할 때는 해당 플랫폼 규정도 준수해야 합니다.
