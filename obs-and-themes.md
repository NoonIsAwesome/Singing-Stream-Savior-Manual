...
關閉自動展示後，預覽會立即回到目前專案的真實狀態；展示歌曲不會寫入專案、已唱紀錄或待播清單。
...
| **已唱** | Set List／History | 字型、字體大小、文字顏色、編號樣式（關閉、1.、01、．）、粗體、斜體、底線、文字對齊，以及清單捲動速度 |
| **待播** | Reserve／Next On | 可與已唱清單分開設定字型、大小、顏色、編號樣式（關閉、1.、01、．）、粗體、斜體、底線與文字對齊 |

### 主題可能限制部分選項

- 標示為**可調整**的主題只會顯示它宣告支援的控制項。
- 標示為**固定設計**的主題會隱藏不適用的字型、顏色、對齊與內部版面設定，避免使用者誤以為按鈕故障。
...
從 **2.1.3.2** 起，「歌單外觀」預覽中的「編輯版面配置」會直接調整 OBS 使用的主題位置與大小。

1. 選擇主題後，按即時預覽右上方的「編輯版面配置」。
2. 拖曳主題外框內部來移動歌單；拖曳角落控制點來等比例縮放。畫布固定為 `1920 × 1080`、`16:9`，不會跟著拉伸。
3. 已加入 OBS 的同一份歌單來源會在拖曳時即時更新，可一邊操作、一邊確認直播版面，不必重新加入來源。

靠近畫布中線時會提供對齊輔助。再次按下按鈕或離開「歌單外觀」即可退出編輯，保留調整結果；旁邊的重設圖示可恢復版面配置。請儲存專案以保留設定。
...
Default 主題預設使用白色預覽背景；透明主題與精緻主題可用透明格確認半透明與留白效果。

- **背景：** 可切換透明格、深色、淺色、自訂顏色或自訂圖片。
- **圖片模式：** 使用自訂圖片時，可選擇符合、填滿或拉伸。

背景與背景圖片只供軟體內檢視，不會輸出到 OBS；「編輯版面配置」所調整的主題位置與大小則會同步 OBS。右上角的 `OBS · 1920 × 1080 · 16:9` 表示輸出畫布。
...
歌詞 Overlay 是選用畫面，與歌單 Overlay 屬於兩個獨立 Browser Source，可分別放置、縮放與顯示。主播也能只使用獨立歌詞視窗而不輸出給觀眾；詳細設定請參考[歌詞章節]({{ '/lyrics.html' | relative_url }})。
