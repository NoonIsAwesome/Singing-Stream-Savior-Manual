...
description: Singing Stream Savior 2.1.4.0 日本語マニュアル
lang: ja
...
Singing Stream Savior は、歌枠配信向けの Windows アプリです。曲ライブラリ、BGM、カラオケ音源、待機リスト、歌詞、ボーカル処理、オーディオルーティング、OBS 出力を一つの操作画面にまとめます。本書は **2.1.4.0** に対応しています。
...
> **音源と再生用途：** 曲ライブラリへの追加は再生項目を作成するだけで、音楽の利用許諾を得るものではありません。素材の出所と利用条件を確認してください。個人での再生、音源分離、配信、録画の保存、出力ファイルの共有では、必要な権利の範囲が異なる場合があります。YouTube の素材には、適用されるプラットフォームの規約も適用されます。
...
ジャケットは必須ではなく、Card と CD テーマで特に効果を発揮します。

> **ジャケット画像について：** 検索機能は画像を探すためのものです。公開配信や動画で使用する前に、必要な利用許諾を確認してください。自作した画像や適切な許諾を得た画像も使用できます。
...
予約が空の状態で予約モードの再生を押しても案内を表示するだけで、BGM は中断しません。曲ライブラリから直接ダブルクリックした場合は自動的に **1 回再生** へ戻ります。「プレイリスト外観」で OBS の予約表示を有効にし、「次の1曲のみ」または 2、3、5、10 曲を選べます。通常終了後は履歴を次回へ持ち越しませんが、異常終了時は復元スナップショットから当日の予約と履歴を復元できます。

<a id="bgm-chat-topic"></a>
### BGM 再生中に雑談テーマを表示する

BGM プレイヤーのリストボタン横にある雑談テーマ設定を開き、BGM 再生中の表示を有効にします。「雑談中」などの見出しと、必要に応じて話題を入力してください。見出しが空の場合は既定の文字が使われます。変更はすぐに反映され、プロジェクトとともに保存されます。

OBS のプレイリストテーマの現在曲表示が雑談の文字に変わるのは、**BGM の再生中だけ**です。BGM の一時停止・停止中は表示されません。伴奏や無伴奏パフォーマンスが進行中の場合は曲名が優先されます。曲名データ、待機リスト、歌唱履歴は変更されません。不要な場合は表示を無効にしてください。

<figure class="manual-figure manual-figure--compact">
  <a href="{{ '/assets/images/bgm-chat-topic.png' | relative_url }}"><img src="{{ '/assets/images/bgm-chat-topic.png' | relative_url }}" alt="BGM 雑談テーマ設定。画像では表示が無効になっています（繁体字中国語 UI）。" loading="lazy" decoding="async"></a>
...
**2.1.3.2** では Basic Lyrics、Classic Karaoke、Vertical Verse の配置を編集できます。「ライブ操作 → 歌詞」でスタイルを選び、プレビュー上部のレイアウト編集ボタンを押します。枠の内側をドラッグすると移動、角のハンドルをドラッグすると縦横比を保った拡大縮小ができます。キャンバスは **1920 × 1080（16:9）** のままです。
...
「プレイリスト外観」でテーマを選び、プレビュー上部のレイアウト編集ボタンを押します。枠の内側をドラッグして移動し、角のハンドルで縦横比を保って拡大縮小します。キャンバスは **1920 × 1080（16:9）** に固定され、同じ出力を使用する OBS ソースには**ドラッグ中から反映**されます。

中央の整列ガイドも利用できます。もう一度ボタンを押すかページを離れると編集が終了し、位置は保持されます。隣のリセットアイコンで配置を戻せます。プロジェクトを保存して設定を残してください。固定デザインのテーマも全体の移動・拡大縮小に対応しますが、内部の書体・色・文字配置は各テーマの対応範囲に従います。

<figure class="manual-figure">
  <a href="{{ '/assets/images/playlist-layout-editor-2132.png' | relative_url }}"><img src="{{ '/assets/images/playlist-layout-editor-2132.png' | relative_url }}" alt="2.1.3.2 のプレイリスト配置編集画面（繁体字中国語 UI）。" loading="lazy" decoding="async"></a>
  <figcaption>2.1.3.2 のプレイリスト配置編集画面（繁体字中国語 UI）。</figcaption>
</figure>
...
OBS Studio 28 以降で「ツール > WebSocket サーバー設定」を開き、サーバーを有効にしてポート（通常 `4455`）とパスワードを確認します。接続情報を閉じたら「OK」を押して保存し、設定画面を閉じます。Singing Stream Savior の「設定 > 配信タイムスタンプ」で WebSocket を有効にし、`127.0.0.1`、同じポートとパスワードを入力して「接続」を押します。OBS のパスワードはローカル接続用の認証情報です。パスワードが写った設定画面を公開しないでください。
...
> **始める前に、素材を利用できる範囲を確認してください。**
>
> 必要な権利を有している素材、音源分離まで許諾されている素材、または法令上利用できる素材を使用してください。音源を購入したこと、インターネットで再生できること、他者から共有されたことだけで、分離・配信・アップロード・配布まで認められるわけではありません。
>
> 本機能は楽曲の利用許諾を提供しません。分離結果にも保護対象の楽曲や録音が含まれる場合があります。公開前に、現在の許諾が予定する用途を含むか確認してください。YouTube の素材には、適用されるプラットフォームの規約も適用されます。
