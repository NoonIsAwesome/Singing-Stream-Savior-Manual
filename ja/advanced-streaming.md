---
title: "高度な配信音声：マイクエフェクトとミックス出力"
description: "2.1.0.0 以降、高度な配信モードでは Singing Stream Savior 内で BGM・伴奏と Profile のエフェクトで処理したマイク音声をミックスし、完全な Stream Mix を OBS・Discord などの配信／通話アプリへ送れます。"
lang: ja
...
<div class="manual-feature-update">
  <div class="manual-feature-update__header"><p class="manual-feature-update__eyebrow">OBS DIRECT OUTPUT</p><h3>OBS 音声プラグインまたは仮想デバイスを使用</h3><p>どちらも Profile、Mix Bus、最終リミッターを通った同じ Stream Output を送ります。</p></div>
...
<div class="manual-feature-update">
  <div class="manual-feature-update__header"><p class="manual-feature-update__eyebrow">2.1.0.0 · AUDIO ROUTING</p><h2 id="route-modes">通常再生と高度な配信モードの違い</h2><p>まず<strong>設定 → オーディオルーティング</strong>でモードを選びます。通常再生はアプリの BGM と伴奏だけを出力します。高度な配信モードではマイク、Profile のエフェクトチェーン、完全なミックス、仮想出力を追加します。</p></div>
...
通常再生は BGM・伴奏、歌詞・セットリスト機能を使用できます。高度な配信モードではさらにマイクを処理してミックスします。Profile はマイクの声を処理し、伴奏を同じボーカルチェーンへ通すわけではありません。Monitor は自分用で、OBS・Discord には Stream Output を送ります。

**マイク → Profile エフェクトチェーン → BGM・伴奏とミックス → Stream Output → OBS / Discord**

<h2 id="singing-profile-defaults">既定の歌唱 Profile と自動切り替え</h2>

「設定 → 音声ルーティング」で歌唱とトークの既定 Profile を選びます。全曲にタグを付ける必要はありません。メイン画面の Profile も手動固定ではなく自動切り替えにします。

{% include profile-introduction.html %}

{% include stream-route-steps.html target="default" %}

[Profile（配信エフェクト）設定へ →]({{ '/ja/profiles.html' | relative_url }})

<h2 id="audio-reference">デバイス・モニター・録音・安定性の詳細</h2>

出力と自動切り替えができたら、必要に応じて以下を確認します。初回の本番前には配信・通話・外部録音を停止して安定性を検査します。毎回の配信前に繰り返す必要はありません。
...
<p id="profile-chain-heading">エフェクトの詳細は独立した Profile 章へ移しました。 <a href="{{ '/ja/profiles.html#profile-chain-heading' | relative_url }}">Profile（配信エフェクト）設定へ →</a></p>
